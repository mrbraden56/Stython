...
private:
  ...
  void stringLiteral()
  {
    ...
    {
    string val = source_code.substr(start, (current)-start + 1);
      advance();
    }
    string temp = source_code.substr(start, (current)-start);
    Tokens.push_back(StringLiteralToken(
        TokenKind::STRING, source_code.substr(start, (current)-start)));
    advance();
  }
  ...
  void numberLiteral()
  {
    ...
    Tokens.push_back(NumberLiteralToken(
        TokenKind::NUMBER, source_code.substr(start, (current)-start)));
  }
  ...
  void identifier()
  {
    ...
    string debug_temp = source_code.substr(start, (current)-start);

    if (reservedWords.find(source_code.substr(start, (current)-start)) !=
        reservedWords.end())
    {
      Tokens.push_back(
          KeywordToken(reservedWords[source_code.substr(start, current)],
                       source_code.substr(start, (current)-start)));
    }
    else
    {
      Tokens.push_back(
  ...
  void scanToken()
  {
    ...
    case ',':
      Tokens.push_back(
          SpecialSymbolToken(TokenKind::COMMA, source_code.substr(start, 1)));
      break;
    case ';':
    ...
    case '-':
      if (lookOneAhead('>'))
      {
        current++; // Increment current if lookOneAhead is true
        Tokens.push_back(
            OperatorToken(TokenKind::RETURN_SYMBOL, source_code.substr(start, (current)-start)));
      }
      else
      {
        Tokens.push_back(
            OperatorToken(TokenKind::MINUS, source_code.substr(start, 1)));
      }

      break;
    case '+':
      Tokens.push_back(
          OperatorToken(TokenKind::PLUS, source_code.substr(start, 1)));
      break;
    case '/':
      Tokens.push_back(
          OperatorToken(TokenKind::SLASH, source_code.substr(start, 1)));
      break;
    case '*':
      Tokens.push_back(
          OperatorToken(TokenKind::STAR, source_code.substr(start, 1)));
      break;
    case '<':
      Tokens.push_back(
          lookOneAhead('=')
              ...
              : OperatorToken(TokenKind::LESS, source_code.substr(start, 1)));
      break;
    case '>':
      Tokens.push_back(
          lookOneAhead('=')
      ...
      Tokens.push_back(
          lookOneAhead('=')
              ? OperatorToken(TokenKind::EQUAL_EQUAL,
                              source_code.substr(start, (current)-start + 1))
              : OperatorToken(TokenKind::EQUAL, source_code.substr(start, 1)));
      break;
    case '!':
      Tokens.push_back(
          lookOneAhead('=')
              ? OperatorToken(TokenKind::BANG_EQUAL,
                              source_code.substr(start, (current)-start + 1))
              : OperatorToken(TokenKind::BANG, source_code.substr(start, 1)));
      break;
    case ' ':
